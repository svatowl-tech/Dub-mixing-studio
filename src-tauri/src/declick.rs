...
/// Reads all samples from any audio file and converts them to normalized f32 interleaved buffer [-1.0, 1.0]
fn read_wav_f32(path: &Path) -> Result<(Vec<f32>, WavSpec), String> {
    ...
    if channels == 0 || sample_rate == 0 {
        return Err("Invalid audio: zero channels or zero sample rate".to_string());
    }
    let spec = WavSpec {
        channels,
        sample_rate,
